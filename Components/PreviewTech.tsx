...
const PreviewTech: FC<PreviewProps> = ({ data }) => {
  ...
  return (
    <div className="grid lg:grid-cols-3 md:grid-cols-2 sm:grid-cols-1 gap-y-8 gap-x-4">
      {data.map((tech, i) => (
        <Link href={`/technology/${tech.platform}/${tech.name}`} key={i} className="p-2 rounded-md flex flex-col gap-2 w-96 bg-PRIMARY">
            ...
            <div className="bg-MAIN rounded-lg p-2 flex flex-col gap-4 h-full justify-between">
              ...
              <ul className="bg-COMPONENT_BG flex gap-4 p-2 rounded-md">
                <li>
                  <a href={tech.repository_url} title="github repo link" target="_blank" rel="noopener noreferrer">
                    <AiFillGithub className="text-blue-500 bg-MAIN p-1 rounded-md text-4xl" />
                  </a>
                </li>
                <li>
                  <a href={tech.homepage} title="homepage" target="_blank" rel="noopener noreferrer">
                    <FiGlobe className="text-blue-500 bg-MAIN p-1 rounded-md text-4xl" />
                  </a>
                </li>
                <li>
                  <a href={tech.package_manager_url} title="package link" target="_blank" rel="noopener noreferrer">
                ...
                <li className="text-blue-500 bg-MAIN p-1 rounded-md flex gap-2 items-center">
                  <FiStar className="text-2xl" /> <span className="text-md text-COMPONENT_BG font-semibold">{addCommasToNumber(tech.stars)}</span>
                </li>
              </ul>
            </div>
